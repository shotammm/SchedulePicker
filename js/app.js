...
-8,America/Los_Angeles,北米 / Los Angeles (UTC-08:00)
-7,America/Denver,北米 / Denver (UTC-07:00)
-6,America/Chicago,北米 / Chicago (UTC-06:00)
-5,America/New_York,北米 / New York (UTC-05:00)
0,Europe/London,UK / London (UTC+00:00)
1,Europe/Berlin,欧州 / Berlin (UTC+01:00)
2,Africa/Johannesburg,アフリカ / Johannesburg (UTC+02:00)
3,Europe/Helsinki,欧州 / Helsinki (UTC+03:00)
5,Asia/Karachi,アジア / Karachi (UTC+05:00)
5.5,Asia/Kolkata,アジア / Kolkata (UTC+05:30)
5.75,Asia/Kathmandu,アジア / Kathmandu (UTC+05:45)
6,Asia/Dhaka,アジア / Dhaka (UTC+06:00)
6.5,Asia/Yangon,アジア / Yangon (UTC+06:30)
7,Asia/Bangkok,アジア / Bangkok (UTC+07:00)
8,Asia/Singapore,アジア / Singapore (UTC+08:00)
9,Asia/Tokyo,アジア / Tokyo (UTC+09:00)
9.5,Australia/Adelaide,オセアニア / Adelaide (UTC+09:30)
10,Australia/Sydney,オセアニア / Sydney (UTC+10:00)
11,Pacific/Noumea,太平洋 / Noumea (UTC+11:00)
12,Pacific/Auckland,太平洋 / Auckland (UTC+12:00)
12.75,Pacific/Chatham,太平洋 / Chatham (UTC+12:45)
13,Pacific/Apia,太平洋 / Apia (UTC+13:00)
14,Pacific/Kiritimati,太平洋 / Kiritimati (UTC+14:00)
`;

let baseMonday = getMonday(dayjs());
let selectedSlots = [];
let hoverCell = null;
...
function renderHeader() {
  document.getElementById("weekLabel").textContent = `${baseMonday.format("YYYY/MM/DD")}週`;
}
...
function buildJstLines() {
  ...
  Object.entries(byDate)
    ...
    .forEach(([date, slots]) => {
      slots.sort((a, b) => a - b);
      const ranges = collectRanges(slots);
      const segs = ranges.map(([s, e]) => formatSlotTime(s, e, outputFormat));
      const d = dayjs(date);
      if (outputFormat === "JP") {
...
function onCalHover(e) {
  const cell = e.target.closest(".cell");
  hoverCell = cell ? { dayIdx: +cell.dataset.day, slotIdx: +cell.dataset.slot } : null;
  if (isDragging) {
  ...
  if (dragMode === "select") {
    for (let d = dayMin; d <= dayMax; d++) {
      ...
      for (let t = slotMin; t <= slotMax; t++) {
        if (!selectedSlots.some((s) => s.date === date && s.slot === t)) added.push({ date, slot: t });
      }
    }
    ...
    for (let d = dayMin; d <= dayMax; d++) {
      const date = baseMonday.add(d, "day").format("YYYY-MM-DD");
      for (let t = slotMin; t <= slotMax; t++) {
        if (selectedSlots.some((s) => s.date === date && s.slot === t)) removed.push({ date, slot: t });
      }
    }
...
function updateFormatToggleText() {
  document.getElementById("outputFormatToggle").textContent = outputFormat;
}
